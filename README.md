# SmolVLA Isaac Sim 工作区

本目录用于承载一个面向 `SmolVLA + Isaac Sim / IsaacLab` 的学习与集成工作区。

当前工作区采用“两层结构”：

- `lerobot/`：上游源码仓库，尽量少改，只在必要时开分支做最小修改。
- `smolvla_isaac_embed/`：你的集成工程，负责启动脚本、配置文件、适配层、安全测试包装层、实验记录与输出。
...
- 创建了 `smolvla_isaac_embed/` 集成工程骨架。
- 创建了中文 `README.md` 与 `AGENTS.md`，用于规范与 AI 的协作方式。
- 为配置、脚本、适配层、安全包装层、实验记录、输出、测试建立了初始目录。

## 为什么这样组织

这样组织的核心目的是：

- 保持上游仓库干净，降低后续同步与升级成本。
- 将你自己的实验性代码放在独立工程中，避免把临时脚本和调试文件堆进 `lerobot`。
- 让 AI 更容易理解“哪些是上游代码，哪些是你的集成层，哪些是你的实验资产”。
- 为后续从“嵌入式直连”过渡到“policy service”或“安全测试平台化”保留结构空间。

## 学习与开发优先级

建议按下面顺序推进，而不是同时学所有东西：

1. 跑通现成的 `SmolVLA + IsaacLab Arena` 闭环。
2. 看懂观测、任务文本、动作输出的接口。
3. 建立你自己的最小适配层与调试脚本。
4. 插入一个最小安全测试模块，例如动作限幅、图像遮挡或延迟注入。

## Markdown 语言约定

除非某个文档明确说明使用英文，否则本工作区内新建的 Markdown 文档默认使用中文书写。
